 ...
 */
public class MocaTraceMetadata extends AbstractTraceMetadataManager {
	...
	public String getTraceTypeName() {
		return MocaConstants.TRACE_TYPE;
	}
		
	public MocaTraceMetadata(SystemDBObject sysDB, String dbName, String alias, int events) throws SoCTraceException {
		super(sysDB);
		this.dbName = dbName;
		this.alias = alias;
		this.events = events;
	...
	public void setTraceFields(Trace trace) {
		...
		trace.setDescription("Moca trace imported " + getCurrentDate());
		trace.setNumberOfCpus(1);
		trace.setNumberOfEvents(events);
		trace.setOutputDevice("Moca");
